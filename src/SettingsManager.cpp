...
bool SettingsManager::updateFromJson(JsonObject &json)
{
    if (!isLoaded)
        load();

    String oldIp = settings.elegooip;
    bool ipChanged = false;

    // Iterate over all known fields and update only those present in the JSON
    for (const auto& field : kSettingFields)
    {
        if (!json.containsKey(field.key))
            continue;

        JsonVariantConst value = json[field.key];

        // Special handling for password - don't update if empty
        if (strcmp(field.key, "passwd") == 0)
        {
            const char* raw = value.as<const char*>();
            if (!raw || strlen(raw) == 0)
                continue;
        ...
        // Apply the value using the same logic as load()
        applyVariant(field, value, settings);

        // Track WiFi changes
        if (strcmp(field.key, "ssid") == 0 || strcmp(field.key, "passwd") == 0)
        {
            wifiChanged = true;
        }
    }
    ...
    // Check if IP changed
    if (settings.elegooip != oldIp && settings.elegooip.length() > 0)
    {
        ipChanged = true;
    }

    // Update logger with new log level if it was changed
